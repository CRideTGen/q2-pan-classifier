...
def create_classifier(
    ...
    train_classifier = ctx.get_action(
    ...
    ref_seqs = qiime2.Artifact.import_data(
        type="FeatureData[Sequence]", view=ref_seqs_file, view_type=None
    )
    ...
    if ref_tax:
        ...
        ref_tax_out = qiime2.Artifact.import_data(
            type="FeatureData[Taxonomy]", view=ref_tax_file, view_type=None
        )
    else:
        ref_tax_out = qiime2.Artifact.import_data(
    ...
    if f_primer and r_primer:
        if min_len and max_len:
            ...
            trimmed_refs = extract_refs(
                sequences=ref_seqs, f_primer=f_primer, r_primer=r_primer
            )

        trained_class = train_classifier(
            reference_reads=trimmed_refs.reads, reference_taxonomy=ref_tax_out
        )
        results += trimmed_refs
    else:
        trained_class = train_classifier(
            reference_reads=ref_seqs, reference_taxonomy=ref_tax_out
        )
        results += [ref_seqs]
